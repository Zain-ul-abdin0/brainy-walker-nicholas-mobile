export default class data {
    static jsonData = [
        {
            id: 0,
            name: 'All',
            selected:false,
            resturants:[
                {
                    id:0,
                    restName: ['Home','Qabail','Cafe','Arabi', 'Khayali',
                        'Sea','GINSOY','MANDAR ','POMELO',
                        'SICHUAN','TAI','XIWANG','DUMPU','GUSTO','MEHAK','LOUNGE','Haveli'],
                    types:'Budget',
                    latlng:[ {
                        latitude: 30.4759034 ,
                        longitude: 72.2789362,
                    },
                    ],
                    image:['https://media-cdn.tripadvisor.com/media/photo-s/15/bb/92/83/park-lane-hotel-lahore.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/0b/64/88/86/the-food.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam',
                    phoneNum:'03135705782',Time:'2:39:00 pm'
                },
            ]
        },
        {
            id: 1,
            name: 'America',
            selected:false,
            resturants:[
                {
                    id:1,
                    restName: ['Arabi', 'Khayali','Sea'],
                    types:'Casual',
                    latlng:[ {
                        latitude: 33.4759034 ,
                        longitude: 73.2789362,
                    },]
                    ,
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/10/c0/90/4f/photo0jpg.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/0b/64/88/86/the-food.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm' ,phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 2,
            name: 'French',
            selected:false,
            resturants:[
                {
                    id:2,
                    restName: ['GINSOY','MANDAR ','POMELO '],
                    types:'Budget',
                    latlng:[ {
                        latitude: 28.4759034 ,
                        longitude:69.2789362,
                    },],
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://blog.e360.pk/wp-content/uploads/2020/01/buffet-restaurant-in-islamabad-scaled.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/10/c0/90/4f/photo0jpg.jpg',
                    ]
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 3,
            name: 'Seafood',
            selected:false,
            resturants:[
                {
                    id:3,
                    restName: ['SICHUAN','TAI'],
                    types:'Casual',
                    latlng:[
                        {
                            latitude: 31.4759034,
                            longitude: 68.2789362 ,
                        },
                    ],
                    image:['https://blizin.com/public/images/uploads/hotel/views/lahorehotel-1578564219.jpg',
                        'https://cf.bstatic.com/images/hotel/max1024x768/247/247297278.jpg',
                        'https://blog.e360.pk/wp-content/uploads/2020/01/buffet-restaurant-in-islamabad-scaled.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',
                    phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 4,
            name: 'Thai',
            selected:false,
            resturants:[
                {
                    id:4,
                    restName: ['XIWANG','DUMPU'],
                    types:'Casual',
                    latlng:[ {
                        latitude: 31.4759034 ,
                        longitude:70.2789362 ,
                    },
                    ],
                    image:['https://ak-d.tripcdn.com/images/200t190000017eala5A8D_Z_550_412_R5_Q70_D.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/0f/5c/5f/c4/photo1jpg.jpg',
                        'https://cf.bstatic.com/images/hotel/max1024x768/247/247297278.jpg'
                    ]
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 5,
            name: 'Deli',
            selected:false,
            resturants:[
                {
                    id:5,
                    restName: ['GUSTO','MEHAK'],
                    types:'Budget',
                    latlng:[ {
                        latitude:31.4759034,
                        longitude:  74.2723362 ,
                    },],
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/1a/64/13/37/the-capital-grill.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/0f/5c/5f/c4/photo1jpg.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 6,
            name: 'Coffee',
            selected:false,
            resturants:[
                {
                    id:6,
                    restName: ['LOUNGE','Haveli'],
                    types:'Budget',
                    latlng:[ {
                        latitude: 31.5102,
                        longitude: 74.3441,
                    },],
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/12/0e/ec/2f/outdoor-sitting-area.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/1a/64/13/37/the-capital-grill.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 7,
            name: 'Mexican',
            selected:false,
            resturants:[
                {
                    id:7,
                    restName: ['CUBE','COVO'],
                    types:'Budget',
                    latlng:[ {
                        latitude: 31.4752034,
                        longitude: 74.2780462 ,
                    },],
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://i.pinimg.com/originals/8a/09/6d/8a096d3b2973674ae93ed66bd98a341f.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/12/0e/ec/2f/outdoor-sitting-area.jpg']
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
        {
            id: 8,
            name: 'Chinese',
            selected:false,
            resturants:[
                {
                    id:8,
                    restName: ['TOKYO','OPTION'],
                    types:'Expensive',
                    latlng:[ {
                        latitude: 36.4759034,
                        longitude:  79.2789362,
                    },],
                    image:['http://jaghajagha.com/gallery.imgs/1706293032209jagha-ambassadorlahore%203.jpg',
                        'https://i.pinimg.com/originals/8a/09/6d/8a096d3b2973674ae93ed66bd98a341f.jpg',
                        'https://media-cdn.tripadvisor.com/media/photo-s/12/0e/ec/2f/outdoor-sitting-area.jpg'
                    ]
                    ,Address: 'Shahrah-e-Quaid-e-Azam'
                    ,Time:'2:39:00 pm',phoneNum:'03135705782'
                },
            ]
        },
    ]
}
